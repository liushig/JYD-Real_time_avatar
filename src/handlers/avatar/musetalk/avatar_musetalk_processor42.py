...
class AvatarMuseTalkProcessor:
    ...
    def __init__(self, avatar: MuseAvatarV15, config: AvatarMuseTalkConfig):
        ...
        # 新增：空闲帧生成控制
        self._idle_frame_enabled = True  # 是否启用空闲帧生成
        self._last_idle_frame_time = 0
        self._idle_frame_interval = 0.04  # 25fps对应的帧间隔
    ...
    def _check_interrupt(self, throw_exception: bool = False) -> bool:
        ...
        if self._interrupt_flag.is_set():
            ...
            return True
        return False
    # 修改 _handle_interrupt_immediate 方法
    def _handle_interrupt_immediate(self):
        ...
        with self._interrupt_lock:
            ...
            # 立即生成多个空闲帧放入_output_queue，确保视频流立即切换到空闲状态
            self._enqueue_multiple_idle_frames()
    ...
    def _enqueue_multiple_idle_frames(self):
        """
        打断后生成并放入多个空闲帧到_output_queue，确保视频流立即切换到空闲状态。
        放入3-5帧，保证帧收集器能立即获取到空闲帧。
        """
        try:
            current_time = time.time()
            num_idle_frames = 5  # 放入5个空闲帧，确保立即切换
            
            for i in range(num_idle_frames):
                # 获取帧ID
                try:
                    frame_id = self._frame_id_queue.get_nowait()
                except queue.Empty:
                    # 如果队列为空，使用当前帧计数器
                    frame_id = self._frame_id_counter + i
                
                # 生成空闲帧
                idle_frame = self._avatar.generate_idle_frame(frame_id)
                ...
                idle_item = {
                    ...
                    'audio_segment': None,
                    'frame_id': frame_id,
                    'timestamp': current_time + i * 0.04  # 模拟25fps的时间戳
                }
                
                # 放入_output_queue
                self._output_queue.put(idle_item)
            
            logger.info(f"[INTERRUPT] Enqueued {num_idle_frames} idle frames after interrupt")
            
        except Exception as e:
            logger.opt(exception=True).error(f"[INTERRUPT] Failed to enqueue idle frames after interrupt: {e}")


    # 修改 _clear_all_queues 方法
    def _clear_all_queues(self):
        """清空所有处理队列，包括输出队列"""
        queues = [
            self._audio_queue, self._whisper_queue, self._unet_queue,
            self._frame_queue, self._frame_id_queue, self._compose_queue,
            self._output_queue  # 关键：清空输出队列！
        ]
        ...
        logger.info("[INTERRUPT] All queues cleared including output queue")

    def _enqueue_idle_frame_after_interrupt(self):
        """
        打断后生成并放入空闲帧到_output_queue，确保视频流连续。
        """
        try:
            # 获取当前时间作为时间戳
            current_time = time.time()
            ...
            # 放入_output_queue
            self._output_queue.put(idle_item)
            logger.info(f"[INTERRUPT] Enqueued idle frame after interrupt, frame_id={frame_id}")
            
        except Exception as e:
            logger.opt(exception=True).error(f"[INTERRUPT] Failed to enqueue idle frame after interrupt: {e}")

    def _clear_internal_queues(self):
        """
        清空所有内部处理队列，但保留_output_queue以保持视频流连续性。
        """
        queues = [
            self._audio_queue, self._whisper_queue, self._unet_queue,
            self._frame_queue, self._compose_queue
        ]
        ...
        logger.info("[INTERRUPT] Internal queues cleared")
    ...
    def add_audio(self, speech_audio: SpeechAudio):
        ...
        # 特殊处理：如果是 end_of_speech 且音频为空，直接触发状态变更
        if len(audio_data) == 0:
            if speech_audio.end_of_speech:
                logger.info(f"Received end_of_speech with empty audio, triggering status change, speech_id={speech_audio.speech_id}")
                # 直接触发 SPEAKING -> LISTENING 状态变更
                self._notify_status_change(speech_audio.speech_id, AvatarStatus.LISTENING)
                return
            else:
    ...
    # 修改 _frame_collector_worker 方法中的打断处理部分
    def _frame_collector_worker(self):
        ...
        while not self._stop_event.is_set():
            ...
            if self._check_interrupt():
                ...
                last_speaking = False
                last_end_of_speech = False
                # logger.info(f"[INTERRUPT] Frame collector handled interrupt at frame_id={local_frame_id}")
            ...
            # 分配帧ID
            try:
                if self._frame_id_queue.empty():
                    self._prefill_frame_id_queue()
                
                frame_id = self._frame_id_queue.get_nowait()
                self._frame_id_counter = frame_id
            except Exception as e:
            ...
            try:
                ...
                # 记录空闲帧生成（调试用）
                if self._config.debug and local_frame_id % 100 == 0:
                    logger.info(f"[IDLE_FRAME] Generated idle frame at frame_id={local_frame_id}")
            ...
            try:
                ...
                logger.opt(exception=True).error(f"Failed to notify video for frame_id={local_frame_id}: {e}")
            ...
            # 语音结束处理
            if end_of_speech and speech_id == self._current_speech_id:
    ...
    def _prefill_frame_id_queue(self):
        """预填充帧ID队列"""
        try:
            current_id = self._frame_id_counter
            for i in range(100):
                self._frame_id_queue.put(current_id + i)
        except Exception as e:
            logger.opt(exception=True).error(f"Failed to prefill frame_id_queue: {e}")
