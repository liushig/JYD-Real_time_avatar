...
class MuseAvatarV15_RealESRGAN:
    def __init__(self, avatar_id, video_path, bbox_shift, batch_size, force_preparation=False,
        ...
        # Face enhancer for super-resolution
        self.face_enhancer = None
        self.use_face_enhancement = True  # Enable face enhancement by default
        self.timesteps = None
        ...
        if self.use_face_enhancement:
            try:
                self.face_enhancer = FaceEnhancer(
                    ...
                    use_realesrgan=True  # Will fallback to traditional methods if not available
                )
                logger.info("Face enhancer initialized successfully")
            except Exception as e:
                logger.warning(f"Failed to initialize face enhancer: {e}, face enhancement disabled")
                self.face_enhancer = None
                self.use_face_enhancement = False
    ...
    def res2combined(self, res_frame, idx):
        ...
        try:
            ...
            # Use INTER_CUBIC for better quality when upscaling from 256x256
            res_frame = cv2.resize(res_frame.astype(np.uint8), (x2 - x1, y2 - y1), interpolation=cv2.INTER_CUBIC)

            # Apply face enhancement to ENTIRE face region (not just mouth)
            # This ensures consistency across the whole face
            if self.use_face_enhancement and self.face_enhancer is not None:
                try:
                    # Enhance entire face region without bbox to maintain consistency
                    res_frame = self.face_enhancer.enhance_mouth_region(res_frame, mouth_bbox=None)
                except Exception as e:
