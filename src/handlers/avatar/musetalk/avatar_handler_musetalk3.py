...
class AvatarMuseTalkContext(HandlerContext):
    ...
    def _media_out_loop(self) -> None:
        ...
        while self.loop_running:
            # 实时检测中断标志，触发则立即停止循环
            if hasattr(self.shared_state, 'avatar_interrupt_flag') and self.shared_state.avatar_interrupt_flag:
                self.loop_running = False
                logger.info("Detected avatar_interrupt_flag=True, stop media out loop")
                break
    ...
    def _event_out_loop(self) -> None:
        ...
        while self.loop_running:
            ...
            if hasattr(self.shared_state, 'avatar_interrupt_flag') and self.shared_state.avatar_interrupt_flag:
                self.loop_running = False
                logger.info("Detected avatar_interrupt_flag=True, stop event out loop")
                break
    ...
    def clear(self) -> None:
        ...
        self.loop_running = False
        if hasattr(self.shared_state, 'avatar_interrupt_flag') and self.shared_state.avatar_interrupt_flag:
            logger.info("Clear context triggered by avatar_interrupt_flag")
...
class HandlerAvatarMusetalk(HandlerBase):
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        """
        # 优先检测中断标志，触发则直接停止接收新数据
        context = cast(AvatarMuseTalkContext, context)
        if hasattr(context.shared_state, 'avatar_interrupt_flag') and context.shared_state.avatar_interrupt_flag:
            logger.info("Detected avatar_interrupt_flag=True, stop receiving new audio data")
            # 清空处理器音频队列，彻底终止待处理任务
            if self.processor and hasattr(self.processor, 'audio_queue'):
                while not self.processor.audio_queue.empty():
                    try:
                        self.processor.audio_queue.get_nowait()
                    except queue.Empty:
                        pass
            return
        ...
        # 切片处理前再次检测中断标志
        if hasattr(context.shared_state, 'avatar_interrupt_flag') and context.shared_state.avatar_interrupt_flag:
            logger.info("Detected avatar_interrupt_flag=True during audio slicing, abort")
            return
        ...
        for audio_segment in slice_data(context.input_slice_context, audio_array.squeeze()):
            # 每个切片提交前检测中断标志
            if hasattr(context.shared_state, 'avatar_interrupt_flag') and context.shared_state.avatar_interrupt_flag:
                logger.info("Detected avatar_interrupt_flag=True, stop submitting audio segments")
                break
        ...
        if speech_end:
            ...
            if end_segment is None:
                end_segment = np.zeros([context.config.output_audio_sample_rate], dtype=np.float32)
                logger.warning(f"Flush returned None, fill with zeros, speech_id: {speech_id}")
            # Wrap end segment and mark as end of speech
            end_speech_audio = SpeechAudio(
                speech_id=speech_id,
                end_of_speech=True,
            ...
            if self.processor:
                self.processor.add_audio(end_speech_audio)
            logger.info(f"Sent end of speech audio for speech_id: {speech_id}")

    def replay_handle(self, speech_id: str, handler_context: HandlerContext):
        """
        Debug method to replay audio data from saved debug cache.
        """
        logger.info(f"Replaying speech_id: {speech_id}")
        debug_root = "logs/audio_segments/"
        debug_file = os.path.join(debug_root, f"{speech_id}.pkl")
        ...
        try:
            ...
            for record in records:
                # Skip if interrupt flag is triggered
                if hasattr(context.shared_state, 'avatar_interrupt_flag') and context.shared_state.avatar_interrupt_flag:
                    logger.info("Replay stopped by avatar_interrupt_flag")
                    break
                if record["inputs"]["type"] != "AVATAR_AUDIO":
                    continue
                # Reconstruct audio array
                audio_data = np.array(record["inputs"]["main_data"], dtype=np.float32)
                speech_id = record["speech_id"]
                speech_end = record["speech_end"]
                # Recreate DataBundle and ChatData
                audio_entry = DataBundleEntry.create_audio_entry(
                    "avatar_muse_audio",
                    1,
                    record["inputs"]["sample_rate"] or context.config.output_audio_sample_rate
                )
                data_bundle = DataBundle(DataBundleDefinition())
                data_bundle.add_entry(audio_entry)
                data_bundle.set_main_data(audio_data)
                data_bundle.set_meta("speech_id", speech_id)
                data_bundle.set_meta("avatar_speech_end", speech_end)
                chat_data = ChatData(type=ChatDataType.AVATAR_AUDIO, data=data_bundle)
                # Re-process the audio
                self.handle(context, chat_data, self.output_data_definitions)
                time.sleep(0.01)
            logger.info(f"Replay completed for speech_id: {speech_id}")
        except Exception as e:
            logger.opt(exception=True).error(f"Failed to replay speech_id {speech_id}: {e}")
