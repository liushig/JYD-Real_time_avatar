...
class AvatarMuseTalkProcessor:
    ...
    def add_audio(self, speech_audio: SpeechAudio):
        ...
        """
###############################################################################################
        #     # 只在一次 speech 的第一段音频记录
        # if speech_audio.is_first:
        #     speech_audio.start_ts = time.perf_counter()
###############################################################################
...
# #################################################################################
#             if is_speaking and not speech_state.first_frame_sent:
#                 end_ts = time.perf_counter()
#                 latency_ms = (end_ts - speech_state.start_ts) * 1000

#                 logger.info(
#                     f"[E2E][MuseTalk] avatar_start_to_stream = {latency_ms:.2f} ms"
#                 )
# ####################################################################################
                # speech_state.first_frame_sent = True
