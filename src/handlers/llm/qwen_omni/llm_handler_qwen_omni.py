...
class QwenOmniConfig(HandlerBaseConfigModel, BaseModel):
    ...
    persona_prompt: Optional[str] = Field(
        default="你不是通义千问，你不是阿里云，你是一名由竞业达开发的数字老师."
    )
...
class HandlerSeq2SeqQwenOmni(HandlerBase, ABC):
    ...
    def start_context(self, session_context, handler_context):
        ...
        try:
            ...
            # 添加角色设定逻辑
            if hasattr(context.config, "persona_prompt") and context.config.persona_prompt:
                try:
                    logger.info(f"Injecting persona message: {context.config.persona_prompt}")
                    context.conversation.add_message(
                        role="user",
                        content=context.config.persona_prompt
                    )
                    context.conversation.add_message(
                        role="assistant",
                        content="好的，我记住了你的设定。"
                    ...
                    logger.warning(f"Failed to inject persona prompt: {e}")
