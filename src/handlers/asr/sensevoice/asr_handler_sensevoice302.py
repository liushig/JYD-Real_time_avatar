        ...
        if rtc_mode == "running":
            ...
            if wakeup_detected:
                if shared is not None:
                    ...
                    logger.info(f"[ASR] rtc_mode=running且检测到唤醒词，置interrupt_flag=True（RTC）和tts_interrupt_flag、avatar_interrupt_flag=True（TTS） | 唤醒词：{wakeup_keyword} | speech_id：{speech_id}")
