...
class Handler(HandlerBase):
    ...
    # ==============================
    # 新增：HUMAN_TEXT替换方法（与RTC风格一致，含日志）
    # ==============================
    def _replace_human_text(self, text: str) -> str:
        ...
        if text != original_text:
            # 超长截断，避免日志刷屏
            trunc_original = original_text[:50] + "..." if len(original_text) > 50 else original_text
            trunc_replaced = text[:50] + "..." if len(text) > 50 else text
            logger.info(f"[ASR-HUMAN_TEXT替换] 原始：{trunc_original} → 替换后：{trunc_replaced}")
        return text
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        # ======================
        # 核心新增：执行文本替换（ASR生成文本的源头）
        # ======================
        output_text = self._replace_human_text(output_text)
