...
class Handler(HandlerBase):
    ...
    # 按钮中断判断和处理
    # ==============================
    def _check_and_process_button_interrupt(self, context: ASRContext, speech_id: str) -> bool:
        """检查按钮中断标志并处理，返回True表示已处理中断"""
        ...
        # 检查按钮中断标志
        # button_interrupt_flag = getattr(shared, 'button_interrupt_flag', False)
        # if button_interrupt_flag:
        #     logger.info(f"[ASR] 检测到按钮中断信号，设置所有中断标志为True | speech_id：{speech_id}")
            
        #     # 设置所有中断标志
        #     shared.interrupt_flag = True  # RTC 使用的打断信号
        #     shared.tts_interrupt_flag = True  # TTS 专用的打断信号
        #     shared.avatar_interrupt_flag = True  # 数字人专用的打断信号
        #     shared.llm_interrupt_flag = True  # LLM 专用的打断信号
            
        #     # 重置按钮中断标志
        #     # shared.button_interrupt_flag = False
        #     logger.info(f"[ASR] 已重置button_interrupt_flag为False | speech_id：{speech_id}")
            
        #     return True
        return False

    # ==============================
    # 状态更新：仅同步current_speech_id
    # ==============================
    def _update_normal_state(self, context: ASRContext, speech_id: str, force_update: bool = False):
        """更新状态：仅同步current_speech_id"""
        shared = context.shared_states
        if shared is None:
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        # 第一步：检查按钮中断
        if self._check_and_process_button_interrupt(context, speech_id):
            logger.info(f"[ASR] 检测到按钮中断，清空音频缓存 | speech_id：{speech_id}")
        ...
        if audio is not None:
            ...
            for audio_segment in slice_data(context.audio_slice_context, audio):
                # 检查按钮中断
                if self._check_and_process_button_interrupt(context, speech_id):
                    logger.info(f"[ASR] 音频收集阶段检测到按钮中断 | speech_id：{speech_id}")
                    context.output_audios.clear()
                    context.audio_slice_context.flush()
                    return
        ...
        # 语音结束后检查按钮中断
        if self._check_and_process_button_interrupt(context, speech_id):
            logger.info(f"[ASR] 语音结束后检测到按钮中断 | speech_id：{speech_id}")
            context.output_audios.clear()
            context.audio_slice_context.flush()
            return
        ...
        # 推理后检查按钮中断
        if self._check_and_process_button_interrupt(context, speech_id):
            logger.info(f"[ASR] 推理后检测到按钮中断，丢弃结果 | speech_id：{speech_id}")
            return
        ...
        # 正常发送文字数据
        logger.info(f"[ASR] 文字生成完成 | 文本：{output_text} | speech_id：{speech_id}")
    ...
    def destroy_context(self, context: HandlerContext):
        if context.shared_states is not None:
            ...
            context.shared_states.button_interrupt_flag = False  # 重置按钮中断标志
