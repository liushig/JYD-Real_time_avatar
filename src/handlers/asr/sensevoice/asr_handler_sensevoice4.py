...
class ASRConfig(HandlerBaseConfigModel, BaseModel):
    ...
    wakeup_words_pattern: str = Field(default=r"^(你)?好\s*[，,]?\s*(|小达|小的|小德|小岛|小当|小唐|小打|一下|等一下|停一下)")
...
class Handler(HandlerBase):
        ...
        if rtc_mode == "running":
            ...
            if wakeup_detected:
                if shared is not None:
                    ...
                    logger.info(f"[ASR] rtc_mode=running且检测到唤醒词，置interrupt_flag=True（RTC）和tts_interrupt_flag、avatar_interrupt_flag、llm_interrupt_flag=True（TTS、数字人、LLM） | 唤醒词：{wakeup_keyword} | speech_id：{speech_id}")  
    ...
    def destroy_context(self, context: HandlerContext):
        if context.shared_states is not None:
            ...
            # 重置interrupt_flag和tts_interrupt_flag，保留rtc_mode
