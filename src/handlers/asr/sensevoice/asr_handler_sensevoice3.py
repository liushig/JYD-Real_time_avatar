...
class ASRConfig(HandlerBaseConfigModel, BaseModel):
    ...
    # 最终版唤醒词正则：仅匹配开头+支持小达/小的/小德+兼容标点/空格
    wakeup_words_pattern: str = Field(default=r"^你好\s*[，,]?\s*(|小达|小的|小德|小岛|小当|小唐|小岛|小打)")
...
# ==========================================================
# 核心 Handler 类（按rtc_mode控制文本拦截+interrupt_flag）
# ==========================================================
class Handler(HandlerBase):
    ...
    def load(self, engine_config: ChatEngineConfigModel,
        ...
        # 兜底正则：同步支持小达/小的/小德（关键修复）
        if self.wakeup_pattern is None:
            self.wakeup_pattern = re.compile(r"^你好\s*[，,]?\s*(小达|小的|小德)", re.IGNORECASE)
    ...
    def _check_wakeup_words(self, text: str) -> Tuple[bool, Optional[str]]:
        ...
        if match:
            keyword = match.group(1) if len(match.groups()) > 0 else match.group(0)
    ...
    # ==============================
    # 3. 中断判断：仅interrupt_flag=True（唤醒词+rtc_mode=running）才中断
    # ==============================
    def _is_interrupted(self, context: ASRContext, speech_id: str) -> bool:
        ...
        # 新增：获取rtc_mode，仅当rtc_mode=running且interrupt_flag=True时才判定为中断
        rtc_mode = getattr(shared, 'rtc_mode', 'ending')
        interrupt_flag = getattr(shared, 'interrupt_flag', False)
        
        if interrupt_flag and rtc_mode == "running":
            logger.info(f"[ASR] 中断判断 | rtc_mode={rtc_mode} + 唤醒词触发interrupt_flag=True，判定为中断 | speech_id：{speech_id}")
            return True
        else:
            logger.info(f"[ASR] 中断判断 | rtc_mode={rtc_mode} + interrupt_flag={interrupt_flag}，不判定为中断 | speech_id：{speech_id}")
            return False
    ...
    # ==============================
    # 5. 核心处理逻辑（按rtc_mode控制文本拦截+interrupt_flag）
    # ==============================
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        # 收集音频（仅rtc_mode=running+interrupt_flag=True时才中断收集）
        if audio is not None:
            audio = audio.squeeze()
            for audio_segment in slice_data(context.audio_slice_context, audio):
                if self._is_interrupted(context, speech_id):
                    logger.info(f"[ASR] 音频收集阶段触发中断（rtc_mode=running+唤醒词） | speech_id：{speech_id}")
        ...
        # 语音结束后检查（仅rtc_mode=running+interrupt_flag=True时才中断）
        if self._is_interrupted(context, speech_id):
            logger.info(f"[ASR] 语音结束后触发中断（rtc_mode=running+唤醒词） | speech_id：{speech_id}")
            context.output_audios.clear()
            context.audio_slice_context.flush()
            return
        ...
        # 推理后检查（仅rtc_mode=running+interrupt_flag=True时才中断）
        if self._is_interrupted(context, speech_id):
            logger.info(f"[ASR] 推理结果无效（rtc_mode=running+唤醒词中断），丢弃 | speech_id：{speech_id}")
            return
        ...
        # 核心逻辑：按rtc_mode分支处理
        # ======================
        rtc_mode = getattr(shared, 'rtc_mode', 'ending') if shared else 'ending'
        wakeup_detected, wakeup_keyword = self._check_wakeup_words(output_text)

        # 分支1：rtc_mode=running → 拦截文本+按唤醒词修改interrupt_flag
        if rtc_mode == "running":
            logger.info(f"[ASR] rtc_mode=running，拦截文本向下流转 | 识别文本：{output_text} | speech_id：{speech_id}")
            
            # 子逻辑：唤醒词文本 → 置interrupt_flag=True；非唤醒词 → 保持False
            if wakeup_detected:
                if shared is not None:
                    shared.interrupt_flag = True
                    logger.info(f"[ASR] rtc_mode=running且检测到唤醒词，置interrupt_flag=True | 唤醒词：{wakeup_keyword} | speech_id：{speech_id}")
            # 非唤醒词不修改interrupt_flag，保持原有False
            
            self._update_normal_state(context, speech_id)
            return  # 直接返回，不传递文本

        # 分支2：rtc_mode=ending → 正常执行所有流程（包括唤醒词文本也传递）
        else:
            logger.info(f"[ASR] 文字生成完成 | 文本：{output_text} | rtc_mode：{rtc_mode}")
            
            # 唤醒词检测仅打印日志，不修改interrupt_flag
            # if wakeup_detected:
            #     logger.info(f"[ASR] 检测到唤醒词但rtc_mode=ending，不修改interrupt_flag | 唤醒词：{wakeup_keyword} | 文本：{output_text}")
