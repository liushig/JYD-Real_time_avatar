# sensevoice模型接收音频流，多次推送，流式
# 切片，每次推送切片，切片长度为16000，1秒
# 等检测到语言结束，再把所有切片拼起来
# 拼接后，调用Sensevoice进行识别
# 以DataBundle的方式返回给系统
...
class Handler(HandlerBase):
    ...
    def _is_interrupted(self, context: ASRContext, speech_id: str) -> bool:
        ...
        logger.info(f"[ASR] speech_id: {speech_id}")
        logger.info(f"[ASR] current_speech_id: {shared.current_speech_id}")

        if shared.current_speech_id != speech_id:
            return True
        if shared.interrupt_flag:
            logger.info(f"[ASR] interrupt_flag: {shared.interrupt_flag}")
            return True
        return False
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        if audio is not None:
            ...
            for audio_segment in slice_data(context.audio_slice_context, audio):
                if self._is_interrupted(context, speech_id):
                    logger.info(f"[ASR] interrupted during audio collect: {speech_id}")
        ...
        # ======================
        if self._is_interrupted(context, speech_id):
            logger.info(f"[ASR] interrupted before generate: {speech_id}")
            context.output_audios.clear()
            context.audio_slice_context.flush()
            return
        ...
        # ======================
        if self._is_interrupted(context, speech_id):
            logger.info(f"[ASR] discard result of interrupted speech: {speech_id}")
            return
