...
# cd /opt/jyd01/wangruihua/JYD_ASR/Qwen3-ASR

# CUDA_VISIBLE_DEVICES=2 python3 -m qwen_asr.cli.demo --asr-checkpoint model/Qwen3-ASR-1.7B --port 7998

# 后台运行

# CUDA_VISIBLE_DEVICES=2 nohup python3 -m qwen_asr.cli.demo --asr-checkpoint model/Qwen3-ASR-1.7B --port 7999 > logs/7999.log 2>&1 &

# 无效命令

# CUDA_VISIBLE_DEVICES=2 python qwen_asr/cli/demo.py --asr-checkpoint model/Qwen3-ASR-1.7B --port 7998

"""
A gradio demo for Qwen3 ASR models.
"""

# Transformers / vLLM 两种后端 指定 GPU 指定语言自动识别 时间戳HTML可视化 HTTPS / 并发控制
...
# 把内部语言code转成人类友好的显示文本，只用于UI显示 不影响模型
...
# 音频处理 ASR的核心工程之一 把任意来源的音频统一成 float32 单声道 这是模型输入稳定性的保障
...
# 统一入口校验函数 没音频 报错，格式不支持 报错，成功 返回（wav,sr）
...
# 命令行参数系统，工程级别设计 必选参数 --asr-checkpoint 
...
"""
        ...
        def run(audio_upload: Any, lang_disp: str, return_ts: bool):
            ...
            results = asr.transcribe(
                audio=audio_obj,
                language="ko",
                prompt="Transcribe only in Korean. Do not output any Chinese or English characters.",
