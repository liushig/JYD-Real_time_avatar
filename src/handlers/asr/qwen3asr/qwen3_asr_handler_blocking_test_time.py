...
class Handler(HandlerBase):
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        vad_duration_ms = audio_bundle.get_meta("vad_duration_ms", 0)  # 获取VAD耗时
        ...
        if not text:
            logger.warning(f"[Qwen3ASRBlocking] ⚠️  空文本 | 耗时: {asr_duration_ms:.2f}ms")
            return

        logger.info(f"[ASR耗时统计] ASR合成总耗时: {asr_duration_ms:.2f}ms | 文本: '{text}'")
        ...
        output.add_meta("vad_duration_ms", vad_duration_ms)  # 传递VAD耗时
        output.add_meta("asr_duration_ms", asr_duration_ms)  # 添加ASR耗时
        ...
        end_output.add_meta("vad_duration_ms", vad_duration_ms)
        end_output.add_meta("asr_duration_ms", asr_duration_ms)
