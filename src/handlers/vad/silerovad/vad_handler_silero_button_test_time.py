...
class HumanAudioVADContext(HandlerContext):
    ...
    def update_status(self, speech_prob: float, clip: np.ndarray,
        ...
        if speech_prob > self.config.speaking_threshold:
            ...
            # 检测到静音，如果之前在说话状态，记录VAD开始时间
            if self.speaking_status == SpeakingStatus.START and self.silence_length == 0:
                # 刚开始进入静音状态，记录时间
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        for clip in slice_data(context.slice_context, audio):
            ...
            if audio_clip is not None:
                ...
                # 如果是语音结束，计算VAD处理耗时
                if human_speech_end and context.vad_start_time is not None:
                    vad_end_time = time.time()
                    vad_duration = (vad_end_time - context.vad_start_time) * 1000
                    output.add_meta("vad_duration_ms", vad_duration)
                    logger.info(f"[VAD耗时统计] VAD处理耗时（最后人声到数据传输）: {vad_duration:.2f}ms")
                    context.vad_start_time = None  # 重置
