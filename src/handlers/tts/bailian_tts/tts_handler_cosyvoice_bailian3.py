...
class HandlerTTS(HandlerBase, ABC):
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        # 检查interrupt_flag，如果为True则立即停止并清空
        # 注意：不重置interrupt_flag，由RTC统一管理重置时机，确保CosyvoiceCallBack也能检测到
        if context.shared_states is not None and context.shared_states.interrupt_flag:
            logger.info(f'检测到interrupt_flag=True，立即停止TTS并清空数据 | speech_id: {speech_id}')
            # 标记当前speech_id为已中断，即使interrupt_flag被重置，CosyvoiceCallBack也能检测到
            ...
            context.current_speech_id = None
            context.input_text = ''
            # 不重置interrupt_flag，让RTC或ASR来统一管理，确保所有异步回调都能检测到
            return
...
class CosyvoiceCallBack(ResultCallback):
    def __init__(self, context: TTSContext, output_definition, speech_id):
        ...
        self.temp_bytes = b''
        self.is_interrupted = False  # 标记是否被中断
    ...
    def on_data(self, data: bytes) -> None:
        # 优先检查当前speech_id是否已被标记为中断（即使interrupt_flag被重置也能检测到）
        if self.context.interrupted_speech_id == self.speech_id:
            logger.info(f'CosyvoiceCallBack检测到speech_id={self.speech_id}已被标记为中断，停止推送并清空数据')
            self.is_interrupted = True
            self.temp_bytes = b''
            return
        
        # 检查interrupt_flag，如果为True则立即停止推送并清空数据
        if self.context.shared_states is not None and self.context.shared_states.interrupt_flag:
            logger.info(f'CosyvoiceCallBack检测到interrupt_flag=True，停止推送并清空数据 | speech_id: {self.speech_id}')
            self.is_interrupted = True
            self.temp_bytes = b''  # 清空已接收的数据
            # 标记当前speech_id为已中断
            if self.context.interrupted_speech_id != self.speech_id:
                self.context.interrupted_speech_id = self.speech_id
            return
        
        # 检查speech_id是否仍然匹配，如果不匹配则停止推送
        if self.context.current_speech_id != self.speech_id:
            logger.info(f'CosyvoiceCallBack检测到speech_id已变更（当前={self.context.current_speech_id}, 回调={self.speech_id}），停止推送旧数据')
            self.is_interrupted = True
            self.temp_bytes = b''
            return
        ...
        if len(self.temp_bytes) > 24000:
            # 再次检查中断标志（双重检查，确保实时性）
            # 优先检查speech_id是否已被标记为中断
            if self.context.interrupted_speech_id == self.speech_id:
                logger.info(f'CosyvoiceCallBack在推送前检测到speech_id={self.speech_id}已被标记为中断，停止推送')
                self.is_interrupted = True
                self.temp_bytes = b''
                return
            
            if self.context.shared_states is not None and self.context.shared_states.interrupt_flag:
                logger.info(f'CosyvoiceCallBack在推送前再次检测到interrupt_flag=True，停止推送 | speech_id: {self.speech_id}')
                self.is_interrupted = True
                self.temp_bytes = b''
                # 标记当前speech_id为已中断
                if self.context.interrupted_speech_id != self.speech_id:
            ...
            # 检查speech_id是否仍然匹配
            if self.context.current_speech_id != self.speech_id:
                logger.info(f'CosyvoiceCallBack在推送前检测到speech_id已变更，停止推送')
                self.is_interrupted = True
                self.temp_bytes = b''
                return
    ...
    def on_complete(self) -> None:
        # 如果已被中断，不发送完成数据
        if self.is_interrupted:
            logger.info(f'CosyvoiceCallBack已被中断，跳过完成回调 | speech_id: {self.speech_id}')
            self.temp_bytes = b''
            return
        
        # 优先检查speech_id是否已被标记为中断
        if self.context.interrupted_speech_id == self.speech_id:
            logger.info(f'CosyvoiceCallBack在完成时检测到speech_id={self.speech_id}已被标记为中断，跳过完成回调')
            self.temp_bytes = b''
            return
        
        # 再次检查interrupt_flag和speech_id
        if self.context.shared_states is not None and self.context.shared_states.interrupt_flag:
            logger.info(f'CosyvoiceCallBack在完成时检测到interrupt_flag=True，跳过完成回调 | speech_id: {self.speech_id}')
            self.temp_bytes = b''
            # 标记当前speech_id为已中断
            if self.context.interrupted_speech_id != self.speech_id:
                self.context.interrupted_speech_id = self.speech_id
        ...
        if self.context.current_speech_id != self.speech_id:
            logger.info(f'CosyvoiceCallBack在完成时检测到speech_id已变更（当前={self.context.current_speech_id}, 回调={self.speech_id}），跳过完成回调')
            self.temp_bytes = b''
            return
