...
class ClientHandlerRtc(ClientHandlerBase):
    ...
    def setup_rtc_ui(self, ui, parent_block, fastapi: FastAPI, avatar_config):
        ...
        frontend_path = Path(DirectoryInfo.get_src_dir() + '/handlers/client/rtc_client/frontend_new3_test_time')
    ...
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        if inputs.data:
            ...
            avatar_first_frame_delay = inputs.data.get_meta("avatar_first_frame_delay_ms", None)
            ...
            if vad_duration is not None:
                ...
                logger.debug(f"[耗时统计] 收集VAD: {vad_duration:.2f}ms")
            if asr_duration is not None:
                delegate.timing_data['asr_duration_ms'] = asr_duration
                logger.debug(f"[耗时统计] 收集ASR: {asr_duration:.2f}ms")
            if llm_first_delay is not None:
                delegate.timing_data['llm_first_delay_ms'] = llm_first_delay
                should_send = True  # LLM首句延迟到达时发送
                logger.info(f"[耗时统计] 收集LLM首句: {llm_first_delay:.2f}ms")
            if tts_duration is not None:
                delegate.timing_data['tts_duration_ms'] = tts_duration
                should_send = True  # TTS数据到达时发送
                logger.info(f"[耗时统计] 收集TTS: {tts_duration:.2f}ms, 数据类型: {inputs.type}")
            if avatar_first_frame_delay is not None:
                delegate.timing_data['avatar_first_frame_delay_ms'] = avatar_first_frame_delay
                should_send = True  # Avatar首帧延迟到达时发送
                logger.info(f"[耗时统计] 收集Avatar首帧: {avatar_first_frame_delay:.2f}ms")

            # 当有新的关键数据时，发送耗时数据到前端
            if should_send:
                # 从 shared_states 获取 DataChannel
                rtc_channel = getattr(delegate.shared_states, 'broadcast_rtc_channel', None) if delegate.shared_states else None
                event_loop = getattr(delegate.shared_states, 'broadcast_event_loop', None) if delegate.shared_states else None

                if rtc_channel and event_loop:
                    import json
                    timing_message = {
                        'type': 'timing_stats',
                    ...
                    try:
                        # 使用 call_soon_threadsafe 在主事件循环中发送
                        def send_timing():
                            try:
                                rtc_channel.send(json.dumps(timing_message))
                                logger.info(f"[耗时统计] 发送到前端: {delegate.timing_data}")
                            except Exception as e:
                                logger.error(f"[耗时统计] DataChannel发送失败: {e}")

                        event_loop.call_soon_threadsafe(send_timing)
                    except Exception as e:
                        logger.error(f"[耗时统计] 发送失败: {e}")
                else:
                    logger.warning(f"[耗时统计] DataChannel未就绪: rtc_channel={rtc_channel is not None}, event_loop={event_loop is not None}")
