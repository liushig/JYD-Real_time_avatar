...
const state = {
  ...
  hasCamera: false, hasMic: false, subtitleTimer: null, typingRowId: null,
...
let remoteVideo, localVideo, localPip, placeholder,
    ...
    subtitleEl, hintBadge, hintText, permOverlay, toast, rightPanel, appEl;
...
function iconCamOn()  { return _svg('<polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/>'); }
function iconCamOff() { return _svg('<line x1="1" y1="1" x2="23" y2="23"/><path d="M21 21H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h3m3-3h6l2 3h4a2 2 0 0 1 2 2v9.34"/>'); }
...
// 新的挂断图标 (横向长条，白色，已移除内联宽高，fill 改为 currentColor)
function iconHangup() {
  return `<svg class="icon" viewBox="0 0 2896 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="12009"><path d="M55.891476 560.95957C55.891476 326.658413 585.156489 1.704008 1416.882996 1.704008c834.112118 0 1360.99152 324.954404 1360.99152 559.255562 0 201.584195 51.120252 503.875288-351.366535 458.548664-402.316386-45.326624-375.563454-201.584195-375.563454-410.666028 0-146.033521-326.828814-178.920883-634.061531-178.920883-309.78873 0-634.231932 32.887362-634.231932 178.920883 0 209.081832 26.923333 365.339404-375.563454 410.666028C2.215211 1064.834858 55.891476 765.099778 55.891476 561.129971z" fill="currentColor" p-id="12010"></path></svg>`;
}

function iconSend()   { return _svg('<line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>'); }
function iconStop()   { return _svg('<rect x="3" y="3" width="18" height="18" rx="2"/>'); }
...
function iconReset()  { return _svg('<path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/><polyline points="22 8 21.5 2 15.5 2.5"/><polyline points="2 16 2.5 22 8.5 21.5"/>'); }

// 字幕显示函数 (支持human和avatar)
function showSubtitle(role, text) {
  let prefix = role === 'human' ? '你: ' : 'AI: ';
  subtitleEl.textContent = prefix + text;
  subtitleEl.classList.add('visible');
  clearTimeout(state.subtitleTimer);
  // 安全超时：30秒后自动隐藏，防止异常
  state.subtitleTimer = setTimeout(function() { subtitleEl.classList.remove('visible'); }, 30000);
}
...
async function accessMedia() {
  if (!navigator.mediaDevices) {
    showToast('请使用 HTTPS 或 localhost 访问', 'error');
    return;
  }
  ...
  if (!stream.getAudioTracks().length) {
    var strack = createSilentAudioTrack();
...
async function startWebRTC() {
  ...
  try {
    ...
    dc.onopen = function() {
      dc.send(JSON.stringify({ type: 'init' }));
    };
...
async function negotiate(pc, webrtcId) {
  ...
  var resp = await fetch('/webrtc/offer', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
function handleDataChannelMessage(event) {
  ...
  if (data.type === 'chat') {
    ...
    // 显示字幕（human 或 avatar）
    if (data.role === 'human') {
      showSubtitle('human', data.message);
    } else {
      showSubtitle('avatar', data.message);
    }
    return;
  }
...
function bindEvents() {
  ...
  chatPanelBtn.addEventListener('click', function() {
    toggleChatPanel();
  });
