...
class RtcClientSessionDelegate(ClientSessionDelegate):
    def __init__(self):
        ...
        # ===== 新增：区分两种文字的延迟配置 =====
        self.human_text_delay: float = 0.0  # 用户输入文字延迟1秒
        self.avatar_text_delay: float = 0.0  # LLM生成文字延迟2秒
    ...
    async def get_data(self, modality: EngineChannelType, timeout: Optional[float] = 0.1) -> Optional[ChatData]:
        ...
        if modality == EngineChannelType.TEXT and data is not None:
            # 判断文字类型：用户输入（HUMAN_TEXT）/LLM生成（AVATAR_TEXT）
            if data.type == ChatDataType.HUMAN_TEXT:
                await asyncio.sleep(self.human_text_delay)  # 用户输入等1秒
            elif data.type == ChatDataType.AVATAR_TEXT:
                await asyncio.sleep(self.avatar_text_delay)  # LLM生成等2秒
...
class ClientRtcConfigModel(HandlerBaseConfigModel, BaseModel):
    ...
    # ===== 新增：可配置的两种文字延迟 =====
    human_text_delay: float = Field(default=0.0, description="用户输入文字呈现延迟（秒）")
    avatar_text_delay: float = Field(default=0.0, description="LLM生成文字呈现延迟（秒）")
