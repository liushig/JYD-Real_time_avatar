...
// ===== 后端API配置 =====
const BACKEND_URL = 'https://agent.jyd.com.cn:8026/';  // 使用localhost
...
async function negotiate(pc, webrtcId) {
  pc.onicecandidate = function(e) {
    if (e.candidate) {
      fetch(BACKEND_URL + '/webrtc/offer', { method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  var resp = await fetch(BACKEND_URL + '/webrtc/offer', { method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function loadConfig() {
  try {
    var resp = await fetch(BACKEND_URL + '/openavatarchat/initconfig');
    if (!resp.ok) return;
