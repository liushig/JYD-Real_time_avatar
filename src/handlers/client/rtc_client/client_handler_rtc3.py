...
class RtcClientSessionDelegate(ClientSessionDelegate):
    def __init__(self):
        ...
        # ===== RTC_MODE 重构：仅基于「实际推流」更新状态 =====
        self.last_push_time: float = 0.0  # 最后一次实际向客户端推流的时间（核心修改）
        self.rtc_mode_idle_threshold: float = 2.0  # 空闲阈值：持续3秒无推流才置为ending
        self.current_rtc_mode: str = "ending"  # 初始状态强制为ending
        self.is_pushing: bool = False  # 标记是否正在推流过程中

    # ===== RTC_MODE 核心重构：仅基于「实际推流」更新状态 =====
    def _update_rtc_mode(self, is_pushing_data: bool, is_business_data: bool = True):
        """
        仅当「实际向客户端推流有效业务数据」时才置为running
        :param is_pushing_data: True=正在推流数据，False=无推流（超时/无数据）
        :param is_business_data: 是否是有效业务数据（过滤心跳/空数据）
        """
        if not self.shared_states:
            logger.warning("[RTC_MODE] shared_states为空，无法更新rtc_mode")
            return
        
        # 1. 正在推流 + 有效业务数据 → 置为running
        if is_pushing_data and is_business_data:
            self.last_push_time = time.time()  # 更新最后推流时间
            self.is_pushing = True
            if self.current_rtc_mode != "running":
                logger.info(f"[RTC_MODE] 状态变更：{self.current_rtc_mode} → running（实际推流有效业务数据）")
                self.current_rtc_mode = "running"
                setattr(self.shared_states, 'rtc_mode', "running")
            return
        
        # 2. 无推流（超时/无数据）→ 防抖判断是否置为ending
        if not is_pushing_data:
            self.is_pushing = False
            # 计算无推流时长
            idle_duration = time.time() - self.last_push_time
            # 仅当持续空闲超过阈值，且当前是running时才更新为ending
            if idle_duration >= self.rtc_mode_idle_threshold and self.current_rtc_mode == "running":
                logger.info(f"[RTC_MODE] 状态变更：running → ending（持续{idle_duration:.1f}秒无实际推流）")
                self.current_rtc_mode = "ending"
                setattr(self.shared_states, 'rtc_mode', "ending")
            return
    ...
    def _is_interrupted(self, speech_id: str = None) -> bool:
        ...
        if interrupt_flag:
            ...
            # 中断时强制置为ending
            self.last_push_time = 0  # 重置推流时间
            self._update_rtc_mode(is_pushing_data=False)
            return True
        
        # 5. 所有speech_id变化仅日志提示，不触发中断
        # if speech_id and getattr(shared, 'current_speech_id', None) != speech_id:
        #     logger.info(f"[RTC] 检测到speech_id变化：{getattr(shared, 'current_speech_id', None)} → {speech_id}（不触发打断）")
        if self.tracking_speech_id and getattr(shared, 'current_speech_id', None) != self.tracking_speech_id:
            logger.info(f"[RTC] 推流speech_id变化：{self.tracking_speech_id} → {getattr(shared, 'current_speech_id', None)}（不触发打断）")
    ...
    async def get_data(self, modality: EngineChannelType, timeout: Optional[float] = 0.1) -> Optional[ChatData]:
        ...
        try:
            ...
            # 超时 → 无推流 → 更新为ending（带防抖）
            self._update_rtc_mode(is_pushing_data=False)
            return None
        
        # ===== 核心修改：获取到数据后，判断是否是有效业务数据，再更新rtc_mode =====
        if data and data.data:
            # 判断是否是有效业务数据
            is_business = self._is_business_data(modality, data.data)
            # 正在推流有效数据 → 更新为running
            self._update_rtc_mode(is_pushing_data=True, is_business_data=is_business)
        else:
            # 空数据 → 无有效推流 → 更新为ending（带防抖）
            self._update_rtc_mode(is_pushing_data=False)
    ...
    # ===== 核心修改：移除put_data中的rtc_mode更新（入队≠推流）=====
    def put_data(self, modality: EngineChannelType, data: Union[np.ndarray, str],
        ...
        # 原有代码：is_business = self._is_business_data(modality, data)
        # 原有代码：self._update_rtc_mode("running", is_business_data=is_business)
...
class ClientRtcConfigModel(HandlerBaseConfigModel, BaseModel):
    ...
    avatar_text_delay: float = Field(default=2, description="LLM生成文字呈现延迟（秒）")
...
class ClientHandlerRtc(ClientHandlerBase):
    ...
    def on_setup_session_delegate(self, session_context: SessionContext, handler_context: HandlerContext,
        ...
        if session_delegate.shared_states:
            ...
            # ===== 关键修改：初始化rtc_mode为ending，且last_push_time置0 =====
            if not hasattr(session_delegate.shared_states, 'rtc_mode'):
                logger.info("[RTC_MODE] 初始化rtc_mode为ending（无实际推流）")
                setattr(session_delegate.shared_states, 'rtc_mode', "ending")
                session_delegate.current_rtc_mode = "ending"
                session_delegate.last_push_time = 0.0  # 初始无推流时间
            else:
                session_delegate.current_rtc_mode = getattr(session_delegate.shared_states, 'rtc_mode', "ending")
                session_delegate.last_push_time = 0.0
    ...
    # ===== 核心修改：移除handle中的rtc_mode更新（入队≠推流）=====
    def handle(self, context: HandlerContext, inputs: ChatData,
        ...
        # ===== 关键删除：移除这里的rtc_mode更新（入队≠推流）=====
        # 原有代码：if inputs.data and inputs.type.channel_type:
        # 原有代码：    is_business = delegate._is_business_data(inputs.type.channel_type, inputs.data)
        # 原有代码：    delegate._update_rtc_mode("running", is_business_data=is_business)
    ...
    def destroy_context(self, context: HandlerContext):
        ...
        if context.client_session_delegate:
            context.client_session_delegate.last_push_time = 0
            context.client_session_delegate._update_rtc_mode(is_pushing_data=False)
            context.client_session_delegate.current_rtc_mode = "ending"
            if context.client_session_delegate.shared_states:
                setattr(context.client_session_delegate.shared_states, 'rtc_mode', "ending")
        pass