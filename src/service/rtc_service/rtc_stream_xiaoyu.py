...
class RtcStream(AsyncAudioVideoStreamHandler):
    ...
    def set_channel(self, channel):
            ...
            async def process_chat_history():
                ...
                while not self.quit.is_set():
                    ...
                    if message == "__AVATAR_END__":
                        end_data = {'type': 'avatar_end'}
                        # 添加conversation_id和问诊进度
                        conversation_id = chat_data.data.get_meta("conversation_id")
                        if conversation_id:
                            end_data['conversation_id'] = conversation_id
                        diagnosis_progress = chat_data.data.get_meta("diagnosis_progress")
                        if diagnosis_progress:
                            end_data['diagnosis_progress'] = diagnosis_progress
                    ...
                    diagnosis_progress = chat_data.data.get_meta("diagnosis_progress")
                    if diagnosis_progress:
                        response_data['diagnosis_progress'] = diagnosis_progress
                        logger.info(f"[RTC推送] 问诊进度: {diagnosis_progress}")

                    self.chat_channel.send(json.dumps(response_data))
            ...
            def _(message):
                ...
                if message['type'] == 'stop_chat':
                    logger.info(f"[RTC] 收到stop_chat，触发打断并重置enable_vad")
                    ...
                    logger.info("[RTC] 已设置所有中断标志")
                elif message['type'] == 'chat':
                    # 检查是否正在回复中
                    if self.client_session_delegate.shared_states.enable_vad is False:
                        logger.warning(f"[RTC] 文本消息被忽略，AI正在回复中: {message['data']}")
                        return
                    ...
                    # 将完整的message转为JSON字符串传递
                    self.client_session_delegate.put_data(
                        EngineChannelType.TEXT,
                        json.dumps(message, ensure_ascii=False),
                        loopback=True
                    )
                    # 注意：avatar_end 会在 Avatar handler 回复完成后自动发送（avatar_handler_musetalk42.py:244）
                # else:
    ...
    async def on_chat_datachannel(self, message: Dict, channel):
        ...
        pass
    def shutdown(self):
        logger.info(f"[RTC清理] 开始清理会话: {self.session_id}")
        self.quit.set()
        factory = None
        ...
        if factory.client_handler_delegate is not None:
            try:
                ...
                logger.info(f"[RTC清理] 会话已停止: {self.session_id}")
            except Exception as e:
                logger.error(f"[RTC清理] 停止会话失败: {e}")

        self.client_session_delegate = None
        logger.info(f"[RTC清理] 清理完成: {self.session_id}")
